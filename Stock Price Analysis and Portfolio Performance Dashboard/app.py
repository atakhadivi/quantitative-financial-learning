...
data = load_data(selected_stock)  # مقداردهی متغیر data
data['Date'] = pd.to_datetime(data['Date'])  # اطمینان از تبدیل درست ستون Date
...
with tab4:
    ...
    portfolio_performance = calculate_portfolio_performance(data)
    st.write(portfolio_performance)  # Adjust based on the script's output format
with tab5:
    st.subheader("Benchmark Comparison")
    benchmark = "SPY"
    benchmark_data = load_data(benchmark)
    benchmark_data = benchmark_data[(benchmark_data['Date'] >= pd.to_datetime(start_date)) & (benchmark_data['Date'] <= pd.to_datetime(end_date))]
    
    comparison_fig = compare_with_benchmark(data, benchmark_data, selected_stock, benchmark)
    st.pyplot(comparison_fig)
with tab6:
    st.subheader("Volatility & Drawdown")
    fig3 = plot_volatility_and_drawdown(data)
    st.pyplot(fig3)